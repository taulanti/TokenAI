...
contract MintTestTokens is Script {
    function run() external {
        ...
        uint256 tokenId2 = aat.mintToAddress(
            testAccount1, deployer, "claude-3", "ml-course-201", uint64(block.timestamp + 60 days), true, true, 300
        );
        ...
        uint256 tokenId3 = aat.mintToAddress(
            testAccount2, deployer, "gpt-4", "web3-course-301", uint64(block.timestamp + 120 days), true, true, 400
        );
        ...
        console.log(
            "- Instructor Pool tokens:", aat.balanceOf(deployer, _computeTokenId("claude-3", "instructor-pool"))
        );
