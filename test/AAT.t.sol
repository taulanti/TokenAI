...
contract AATTest is Test {
    ...
    function testMintSameTokenMultipleTimes() public {
        ...
        vm.startPrank(owner);
        uint256 tokenId1 =
            aat.mintToAddress(user1, originPool, MODEL, SCOPE, expiration, RECLAIMABLE, TRADABLE, amount1);
        uint256 tokenId2 =
            aat.mintToAddress(user2, originPool, MODEL, SCOPE, expiration, RECLAIMABLE, TRADABLE, amount2);
        vm.stopPrank();
    ...
    function testTradeWithNativeFees() public {
        ...
        vm.startPrank(owner);
        uint256 tokenIdA =
            aat.mintToAddress(user1, originPool, MODEL, "course-a", expiration, RECLAIMABLE, TRADABLE, 1000);
        uint256 tokenIdB =
            aat.mintToAddress(user2, originPool, MODEL, "course-b", expiration, RECLAIMABLE, TRADABLE, 800);
        vm.stopPrank();
