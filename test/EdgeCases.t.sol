...
contract EdgeCasesTest is Test {
    ...
    function testFeeOverflowPrevention() public {
        ...
        vm.prank(user1);
        tokenAI.approve(address(aat), type(uint256).max);

        // Try excessive native fee
        vm.prank(owner);
        vm.expectRevert();
        aat.transfer(user1, user2, tokenId, 100, type(uint256).max); // Excessive native fee
    }
    ...
    function testTimestampUnderflowPrevention() public {
        ...
        vm.prank(owner);
        uint256 tokenId = aat.mintToAddress(user1, originPool, MODEL, SCOPE, earlyExpiration, true, true, 1000);
        ...
        aat.transfer(user1, user2, tokenId, 100, 0);
    }
    ...
    function testReentrancyDuringTransfer() public {
        ...
        ReentrantReceiver maliciousContract = new ReentrantReceiver(aat, owner);

        // Transfer TO malicious contract - this will trigger the reentrant call
        vm.prank(owner);
        aat.transfer(user1, address(maliciousContract), tokenId, 100, 0);
    ...
    function testExcessiveFeeAttack() public {
        ...
        vm.prank(user1);
        tokenAI.approve(address(aat), 100 * 10 ** 18); // Approve more than balance

        // Try to set fee larger than user's TokenAI balance
        vm.prank(owner);
        vm.expectRevert();
        aat.transfer(user1, user2, tokenId, 100, 100 * 10 ** 18); // Excessive native fee
    }
    ...
    function testTokenConfigurationMismatch() public {
        ...
        vm.prank(owner);
        uint256 tokenId1 = aat.mintToAddress(user1, originPool, MODEL, SCOPE, expiration, true, true, 1000);

        // Try to mint same token ID with different config (should fail)
        vm.prank(owner);
        uint256 tokenId2 = aat.mintToAddress(user1, originPool, MODEL, SCOPE, expiration, true, true, 500);

        // Should be same token ID
        assertEq(tokenId1, tokenId2);
        assertEq(aat.totalSupply(tokenId1), 1500);
    }

    function testNonTradableTokenBypass() public {
        // Mint non-tradable token to user (not origin pool)
        vm.prank(owner);
        uint256 tokenId = aat.mintToAddress(user1, originPool, MODEL, SCOPE, expiration, true, false, 1000);
        ...
        vm.prank(owner);
        aat.mintToAddress(originPool, originPool, MODEL, SCOPE, expiration, true, false, 500);

        vm.prank(owner);
        aat.transfer(originPool, user1, tokenId, 100, 0);
    }
    ...
    function testLargeBatchOperationGasLimit() public {
        ...
        assertEq(aat.balanceOf(recipients[0], tokenId), 1);
        assertEq(aat.balanceOf(recipients[arraySize - 1], tokenId), 1);
    }
    ...
    function testTradeFrontRunning() public {
        ...
        vm.startPrank(owner);
        uint256 tokenIdA = aat.mintToAddress(user1, originPool, MODEL, "course-a", expiration, true, true, 1000);
        uint256 tokenIdB = aat.mintToAddress(user2, originPool, MODEL, "course-b", expiration, true, true, 800);
        vm.stopPrank();
        ...
        vm.prank(user1);
        tokenAI.approve(address(aat), 50 * 10 ** 18);
        vm.prank(user2);
        tokenAI.approve(address(aat), 50 * 10 ** 18);

        // Simulate front-running by changing fees mid-transaction
        vm.prank(owner);
        aat.tradeWithNativeFees(user1, user2, tokenIdA, 100, tokenIdB, 80, 0, 10 * 10 ** 18, 15 * 10 ** 18);
    ...
    function testExpirationBoundaryConditions() public {
        ...
        vm.prank(owner);
        uint256 tokenId = aat.mintToAddress(user1, originPool, MODEL, SCOPE, currentTime, true, true, 1000);
        ...
        vm.prank(owner);
        uint256 tokenId2 = aat.mintToAddress(user1, originPool, MODEL, "scope-2", currentTime + 1, true, true, 1000);

        // Should not be expired yet
        assertFalse(aat.isExpired(tokenId2));

        // Move forward one second
        vm.warp(block.timestamp + 1);

        // Now should be expired
        assertTrue(aat.isExpired(tokenId2));
    }

    function testZeroExpirationToken() public {
        // Token with zero expiration should never expire
        vm.prank(owner);
        uint256 tokenId = aat.mintToAddress(user1, originPool, MODEL, SCOPE, 0, true, true, 1000);

        assertFalse(aat.isExpired(tokenId));
    ...
    function testZeroAddressInputs() public {
        ...
        aat.mintToAddress(address(0), originPool, MODEL, SCOPE, expiration, true, true, 1000);

        // Test zero origin pool
        vm.expectRevert(AAT.ZeroAddress.selector);
        aat.mintToAddress(user1, address(0), MODEL, SCOPE, expiration, true, true, 1000);

        // Test zero fee recipient
        vm.expectRevert(AAT.ZeroAddress.selector);
        aat.setTreasury(address(0));
    ...
    function _mintToken(address to, uint256 amount) internal returns (uint256 tokenId) {
        vm.prank(owner);
        tokenId = aat.mintToAddress(to, originPool, MODEL, SCOPE, expiration, true, true, amount);
    }
}
...
contract ReentrantReceiver {
    ...
    constructor(AAT _aat, address _owner) {
        aat = _aat;
        owner = _owner;
    }
        ...
        returns (bytes4)
    {
        ...
        if (!attacked) {
            attacked = true;
            try aat.transfer(address(this), from, id, 50, 0) {
