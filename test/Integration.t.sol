...
contract IntegrationTest is Test {
    ...
    function testStudentToStudentTrading() public {
        ...
        vm.startPrank(owner);
        uint256 ai101TokenId =
            aat.mintToAddress(student1, instructor, MODEL_GPT4, COURSE_AI_101, expiration, true, true, 300);
        uint256 ml201TokenId =
            aat.mintToAddress(student2, instructor, MODEL_CLAUDE, COURSE_ML_201, expiration, true, true, 200);
        vm.stopPrank();
    ...
    function testInstructorDistributionFlow() public {
        ...
        vm.prank(owner);
        uint256 tokenId =
            aat.mintToAddress(instructor, instructor, MODEL_GPT4, COURSE_AI_101, expiration, true, true, totalCredits);
    ...
    function testCrossCourseNativeTrading() public {
        ...
        vm.startPrank(owner);
        uint256 ai101TokenId =
            aat.mintToAddress(student1, instructor, MODEL_GPT4, COURSE_AI_101, expiration, true, true, 500);
        uint256 ml201TokenId =
            aat.mintToAddress(student2, instructor, MODEL_CLAUDE, COURSE_ML_201, expiration, true, true, 400);
        vm.stopPrank();
    ...
    function testPlatformEmergencyControls() public {
        ...
        vm.expectRevert();
        aat.mintToAddress(
            student1, instructor, MODEL_GPT4, COURSE_AI_101, uint64(block.timestamp + 30 days), true, true, 100
        );
    ...
    function testMultiModelCreditManagement() public {
        ...
        uint256 gpt4TokenId =
            aat.mintToAddress(student1, instructor, MODEL_GPT4, COURSE_AI_101, expiration, true, true, 300);
        uint256 claudeTokenId =
            aat.mintToAddress(student1, instructor, MODEL_CLAUDE, COURSE_AI_101, expiration, true, true, 200);
        vm.stopPrank();
    ...
    function _setupBasicCredits() internal returns (uint256 tokenId) {
        ...
        tokenId = aat.mintToAddress(
            student1, instructor, MODEL_GPT4, COURSE_AI_101, uint64(block.timestamp + 30 days), true, true, 1000
        );
    }
    ...
    function testComplexMultiUserScenario() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            vm.prank(owner);
            aat.tradeWithNativeFees(
                users[userA], users[userB], tokenIds[tokenA], 50, tokenIds[tokenB], 40, 0, 2 * 10 ** 18, 3 * 10 ** 18
            );
        }
